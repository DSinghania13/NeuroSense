...
adds environmental noise, tests mixed clinical cases,
and supports direct analysis of real human audio files.
"""
...
from pydub.generators import WhiteNoise  # 🔥 NEW: For simulating noisy inputs
...
# ==========================================
# 🔥 UPDATED: Scenarios are now configuration dictionaries to support Real Audio & Noise
SCENARIOS = {
    "A_Healthy_Speech": {
        "mode": "synthesize",
        "noise_db": None,
        "segments": [
            ...
            ("We talked for a while about different things and shared some ideas. After that I came back home, had dinner, and spent some time watching television before going to sleep.",
             0.0)
        ]
    },
    "B_MCI_Speech": {
    ...
    "C_Repetition_Loop": {
        ...
        "segments": [
            ...
            ("I kept repeating the exact same thing every single time. I just went there again and again and again to look around before finally going home to rest.",
             0.0)
        ]
    },
    "D_Filler_Heavy": {
        ...
        "segments": [
            ...
            ("Then I stayed home and did not do much after that, just watched television for the rest of the entire day.",
             0.0)
        ]
    },
    "E_Topic_Drift": {
    ...
    # 🔥 NEW: Tests Anomia + Repetition + Mild Incoherence WITH background noise
    "F_Mixed_Case_Noisy": {
        "mode": "synthesize",
        "noise_db": -25.0,
        "segments": [
            ("I was trying to find the, um, remote control.", 0.5),
            ("The television was not working, so I looked for the remote control everywhere in the house.", 0.5),
            ("I checked under the pillows and on the shelf. Then I decided to go outside and water the plants because it was raining.", 0.8),
            ("I came back inside the living room and the remote control was just sitting right there on the chair the whole time.", 0.0)
        ]
    },
    # 🔥 NEW: Skips TTS completely and runs a real human audio file through the pipeline
    "G_Real_Patient_Audio": {
        "mode": "real",
        "filepath": "real_sample.wav"  # <-- Drop a real human sample in your root folder and change this name!
    }
}


# ==========================================
# 2. AUDIO GENERATOR (Now with Noise support)
# ==========================================
def generate_simulation_audio(segments, output_filename, noise_db=None):
    ...
    # 🔥 Overlay environmental noise if requested
    if noise_db is not None:
        print(f"   🌪️ Overlaying background noise at {noise_db}dB...")
        noise = WhiteNoise().to_audio_segment(duration=len(combined_audio))
        noise = noise + noise_db  # pydub handles volume reduction via addition of negative DB
        combined_audio = combined_audio.overlay(noise)
...
# ==========================================
# 3. REPORTING ENGINE (Unchanged, it's perfect)
# ==========================================
...
def print_detailed_table(result):
    risk_level = result.get("final_score", {}).get("summary", {}).get("risk_level")
    if risk_level == "invalid":
        print("\n⛔ Sample too small — skipping detailed table.")
        return
...
# ==========================================
# 4. MAIN EXECUTION (Updated for Logic Branching)
# ==========================================
def run_full_suite():
    ...
    for label, config in SCENARIOS.items():
        mode = config.get("mode", "synthesize")

        # --- BRANCH A: Synthetic TTS Data ---
        if mode == "synthesize":
            filename = f"{label}.mp3"
            generate_simulation_audio(config["segments"], filename, config.get("noise_db"))
            target_file = filename

        # --- BRANCH B: Real Human Audio Data ---
        else:
            target_file = config.get("filepath")
            if not target_file or not os.path.exists(target_file):
                print(f"\n{'=' * 80}")
                print(f"⚠️ SKIPPING {label}: Could not find real audio file at '{target_file}'")
                print(
                    "   (Drop a .wav or .mp3 file in this folder and update the path in SCENARIOS to test real human speech!)")
                print(f"{'=' * 80}")
                continue

            print(f"\n   🎙️ Sourced Real Audio File: {target_file}")

        # Execute Pipeline
        result = service.analyze_audio(target_file)

        # Report
        print_ui_report(label, result)
        print_detailed_table(result)

        # Cleanup ONLY if we generated it (Don't delete the user's real file!)
        if mode == "synthesize":
